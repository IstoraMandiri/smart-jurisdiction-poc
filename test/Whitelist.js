...
contract('Whitelist', function (addresses) {
  ...
  describe('set / get', function () {
    it('returns false for undefined values', async function () {
      assert.equal(await whitelist.whitelist.call(addresses[1]), false);
      assert.equal(await whitelist.whitelist.call(addresses[2]), false);
    });
    it('allows the owner to update it', async function () {
      await whitelist.set(addresses[1], true);
      await whitelist.set(addresses[2], true);
      assert.equal(await whitelist.whitelist.call(addresses[1]), true);
      assert.equal(await whitelist.whitelist.call(addresses[2]), true);
    });
